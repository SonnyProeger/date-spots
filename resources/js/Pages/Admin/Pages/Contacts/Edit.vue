			...
			<form @submit.prevent="update">
				<div class="flex flex-wrap -mb-8 -mr-6 p-8">
					...
					<text-input v-model="form.last_name" :error="form.errors.last_name" class="pb-8 pr-6 w-full lg:w-1/2"
					            label="Last name"/>
					<select-input v-model="form.datespot_id" :error="form.errors.datespot_id" class="pb-8 pr-6 w-full lg:w-1/2"
					              label="Datespot">
						<option :value="null"/>
						<option v-for="datespot in datespots" :key="datespot.id" :value="datespot.id">{{ datespot.name }}</option>
					</select-input>
	...
	data() {
		return {
			form: this.$inertia.form({
				...
				datespot_id: this.contact.datespot_id,
