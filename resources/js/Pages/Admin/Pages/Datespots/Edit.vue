<template>
	<div>
		...
		<h1 class="mb-8 text-3xl font-bold">
			<Link class="text-indigo-400 hover:text-indigo-600" href="/datespots">Datespots</Link>
		...
		<div class="mt-6 bg-white rounded shadow overflow-x-auto">
			<table class="w-full whitespace-nowrap">
				...
				<tr v-for="contact in datespot.contacts" :key="contact.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
					<td class="border-t">
				...
				<tr v-if="datespot.contacts.length === 0">
...
export default {
	...
	data() {
		return {
			form: this.$inertia.form({
				...
				phone: this.datespot.phone,
				address: this.datespot.address,
				city: this.datespot.city,
				region: this.datespot.region,
				country: this.datespot.country,
	...
	methods: {
		update() {
			this.form.put(`/datespots/${this.datespot.id}`)
		},
		destroy() {
			if (confirm('Are you sure you want to delete this datespot?')) {
				this.$inertia.delete(`/datespots/${this.datespot.id}`)
			}
		},
		restore() {
			if (confirm('Are you sure you want to restore this datespot?')) {
				this.$inertia.put(`/datespots/${this.datespot.id}/restore`)
			}
		},
	},
}
